...
type TimeEntry = { time: string; item: PositionedText; x: number; order: number };
...
function flattenLines(result: OcrTextResult): OcrLine[] {
  return result.blocks.flatMap((block) => (
    block.lines?.length ? block.lines : [{ text: block.text, frame: block.frame }]
  ));
}
...
function uniqueNameCandidates(items: PositionedText[], configuredName: string): NameCandidate[] {
  const candidates = items
    .map((item) => ({
      item,
      score: nameScore(item.text, configuredName),
  ...
  for (const candidate of candidates) {
    const duplicate = unique.some((existing) => (
      ...
      && Math.abs(centerY(existing.item.frame) - centerY(candidate.item.frame))
        <= Math.max(height(existing.item.frame), height(candidate.item.frame))
    ));
...
function selectName(items: PositionedText[], configuredName: string) {
  ...
  if (!best) {
    return {
      item: null,
      warning: 'No encontré el nombre configurado. Revisa cómo apareces en la planilla o usa una captura más nítida.',
    };
  }
  if (best.score < 80) {
    return {
...
function dayHeaderIndex(text: string) {
  ...
  return DAY_WORDS.findIndex((variants) => variants.some(
    (variant) => value === variant || value.includes(variant),
  ));
}
...
function clusterRows(items: PositionedText[], threshold: number): RowCluster[] {
  ...
  for (const item of sorted) {
    ...
    target.center = target.items.reduce((sum, current) => sum + centerY(current.frame), 0)
      / target.items.length;
  }
...
function fittedDayCenters(headers: Map<number, PositionedText>) {
  ...
  const numerator = points.reduce(
    (sum, point) => sum + (point.day - meanDay) * (point.x - meanX),
    0,
  );
...
function timeEntries(rowItems: PositionedText[]): TimeEntry[] {
  return rowItems
    .flatMap((item) => (
      parseTimes(item.text).map((time, order) => ({
        time,
        item,
...
function readDaySlots(rowItems: PositionedText[], dayCenter: number, dayGap: number): DaySlots {
  ...
  const entries = timeEntries(rowItems)
...
function readSequentialWeek(rowItems: PositionedText[]): ReviewShift[] | null {
  const entries = timeEntries(rowItems);
  if (entries.length < 21) return null;

  // Schedules handled by WeekFlow use exactly three values per day:
  // entry, break duration and exit. Any weekly total is to the right and is ignored.
  const weekTimes = entries.slice(0, 21).map((entry) => entry.time);
  const shifts = DAYS.map((_, day) => {
    const offset = day * 3;
    const slots: DaySlots = {
      start: weekTimes[offset] ?? null,
      breakTime: weekTimes[offset + 1] ?? null,
      end: weekTimes[offset + 2] ?? null,
      sourceText: weekTimes.slice(offset, offset + 3).join(' '),
    };
    return fromStructuredSlots(day, slots, 'high');
  });

  return shifts.every((shift) => !shift.issue) ? shifts : null;
}

function issueCount(shifts: ReviewShift[]) {
  return shifts.filter((shift) => Boolean(shift.issue) || shift.confidence === 'low').length;
}

export function parseScheduleOcr(
  ...
  const candidateRows = clusters.filter(
    (cluster) => cluster.items.reduce(
      (count, item) => count + parseTimes(item.text).length,
      0,
    ) >= 2,
  );
  ...
  const selectedIndex = candidateRows
    .map((cluster, index) => ({
      index,
      distance: Math.abs(cluster.center - nameY),
    }))
  ...
  if (
    !selectedRow
    || Math.abs(selectedRow.center - nameY) > Math.max(typicalHeight * 2.5, 60)
  ) {
    return {
      nameFound: true,
      matchedNameText: best.text,
      rowText: '',
      shifts: DAYS.map((_, day) => emptyShift(
        day,
        'Encontré tu nombre, pero no pude asociarlo con una fila de horarios con seguridad.',
      )),
      warnings: [
        'Encontré tu nombre, pero la fila de horas quedó demasiado lejos o incompleta. No inventé ningún horario.',
      ],
    };
  }
  ...
  const nextRow = selectedIndex < candidateRows.length - 1
    ? candidateRows[selectedIndex + 1]
    : null;
  const topBound = previousRow
    ? (previousRow.center + rowCenter) / 2
    : rowCenter - typicalHeight;
  const bottomBound = nextRow
    ? (rowCenter + nextRow.center) / 2
    : rowCenter + typicalHeight;
  ...
  if (fit && headers.size >= 2) {
    ...
    shifts = DAYS.map((_, day) => emptyShift(
      day,
      'No pude ubicar esta columna con seguridad.',
      rowText,
    ));
  }

  const sequentialWeek = readSequentialWeek(rowItems);
  if (sequentialWeek && issueCount(sequentialWeek) < issueCount(shifts)) {
    shifts = sequentialWeek;
    warnings.push(
      'La geometría de algunas celdas venía comprimida. Reconstruí tu fila completa de izquierda a derecha como entrada, colación y salida, sin usar el total semanal.',
    );
  } else if (fit && headers.size >= 2) {
    warnings.push(
      'Leí cada día dentro de su bloque real y ordené sus horas como entrada, colación y salida. El total semanal quedó fuera.',
    );
  } else {
    warnings.push(
      'No pude reconstruir las columnas de la semana con seguridad. Dejé los días pendientes en vez de inventar horarios.',
    );
  }

  if (shifts.some((shift) => shift.confidence !== 'high' || shift.issue)) {
    warnings.push(
      'Hay días pendientes de revisión manual antes de guardar. Toca o reescribe sus datos para confirmarlos.',
    );
  }
